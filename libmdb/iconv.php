...
function mdb_iconv_init($mdb)
{
	/* check environment variable */
	if (!(defined("MDBICONV") && $iconv_code = constant("MDBICONV"))) {
		$iconv_code = "UTF-8";
	}

#ifdef HAVE_ICONV
	if (!IS_JET3($mdb)) {
    iconv_set_encoding("input_encoding", $iconv_code);
    iconv_set_encoding("output_encoding", "UCS-2LE");
		...
		/* check environment variable */
		if (!(defined("MDBICONV") && $jet3_iconv_code = constant("MDB_JET3_CHARSET"))) {
			$jet3_iconv_code = "CP1252";
		}

    iconv_set_encoding("input_encoding", $iconv_code);
    iconv_set_encoding("output_encoding", $jet3_iconv_code);
	}
#endif
}
